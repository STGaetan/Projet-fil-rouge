...
const navItems: NavItem[] = [
  ...
  { name: "Stagiaires", path: "/stagiaires",  Icon: Users },
  { name: "Formations", path: "/formations",  Icon: BookOpen },
  { name: "Dossiers",   path: "/dossiers",    Icon: FolderOpen },
  { name: "Absences",   path: "/absences",    Icon: Clock },
];
...
export function AdminLayout() {
  ...
  const getPageTitle = () => {
    switch (location.pathname) {
      case "/":           return { title: "Vue d'ensemble",  subtitle: "Gestion des dossiers et suivi" };
      case "/stagiaires": return { title: "Stagiaires",       subtitle: "Annuaire et suivi" };
      case "/formations": return { title: "Formations",       subtitle: "Gestion des cursus" };
      case "/dossiers":   return { title: "Dossiers",         subtitle: "Gestion documentaire" };
      case "/absences":   return { title: "Absences",         subtitle: "Suivi de l'assiduité" };
      default:            return { title: "Espace Admin",     subtitle: "MNS Metz Numeric School" };
    }
  };
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-[#F5F5F5] font-sans overflow-hidden text-[#1A1F3D]">
      ...
      >
        {/* Logo */}
        <div className="h-[70px] lg:h-[80px] flex items-center justify-between px-6 lg:justify-center border-b border-white/10 shrink-0">
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="MNS" className="h-8 lg:h-10 w-auto" />
            <span className="text-sm font-normal text-gray-400 hidden lg:inline">Admin</span>
          </div>
          <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full relative">
        ...
        <header className="h-[70px] lg:h-[80px] bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-8 shrink-0 relative z-10">
          <div className="flex items-center gap-3 lg:gap-4">
            {/* Hamburger visible seulement sur tablette (lg masqué via bottom-nav sur mobile) */}
            <button
            ...
            <div className="truncate">
              <h1 className="text-lg lg:text-2xl font-bold text-[#1A1F3D] truncate">{title}</h1>
              <p className="text-xs lg:text-sm text-gray-500 hidden sm:block truncate">{subtitle}</p>
            </div>
          </div>

          <div className="flex items-center gap-2 lg:gap-6 shrink-0">
            {/* Session badge — masqué sur mobile */}
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[#FF6600] rounded-full border-2 border-white box-content" />
              </button>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-xl shadow-xl border border-gray-100 z-50 animate-in fade-in zoom-in-95 overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/80">
                      <h3 className="font-bold text-[#1A1F3D]">Notifications</h3>
                      <button className="text-xs text-[#FF6600] font-semibold hover:underline">
                        Tout marquer comme lu
                      </button>
                    </div>
                    <div className="max-h-[360px] overflow-y-auto">
                      <div className="p-3 border-b border-gray-50 hover:bg-gray-50 transition-colors flex gap-3 items-start cursor-pointer">
                        <div className="p-1.5 bg-orange-50 text-[#FF6600] rounded-full mt-0.5 shrink-0">
                          <AlertCircle size={16} />
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-[#1A1F3D] leading-tight">Dossier en attente</p>
                          <p className="text-xs text-gray-600 mt-1">Alan Turing n'a pas encore fourni sa convention de stage.</p>
                          <p className="text-[10px] text-gray-400 mt-1.5">Il y a 10 min</p>
                        </div>
                      </div>
                      <div className="p-3 border-b border-gray-50 hover:bg-gray-50 transition-colors flex gap-3 items-start cursor-pointer">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[#1A1F3D] leading-tight">Nouvelle absence signalée</p>
                          <p className="text-xs text-gray-600 mt-1">Marie Curie est signalée absente aujourd'hui (Matin).</p>
                          <p className="text-[10px] text-gray-400 mt-1.5">Il y a 1 heure</p>
                        </div>
                      </div>
                      <div className="p-3 hover:bg-gray-50 transition-colors flex gap-3 items-start cursor-pointer">
                        <div className="p-1.5 bg-green-50 text-green-600 rounded-full mt-0.5 shrink-0">
                          <CheckCircle2 size={16} />
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-[#1A1F3D] leading-tight">Dossier validé</p>
                          <p className="text-xs text-gray-600 mt-1">Le dossier de Grace Hopper a été vérifié et validé.</p>
                          <p className="text-[10px] text-gray-400 mt-1.5">Hier</p>
                        </div>
                      </div>
                    </div>
                    <div className="p-3 border-t border-gray-100 text-center bg-gray-50/50 hover:bg-gray-100 transition-colors cursor-pointer">
                      <span className="text-sm font-semibold text-[#1A1F3D]">Voir toutes les notifications</span>
                    </div>
                  </div>
                </>
              )}
            ...
            <div className="relative">
              ...
              >
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-bold text-[#1A1F3D] leading-none">{user?.nom ?? "Admin"}</p>
                  <p className="text-xs text-gray-500 mt-1 truncate max-w-[100px]">{user?.email}</p>
                </div>
                <ImageWithFallback
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 min-w-[200px] bg-white rounded-xl shadow-lg border border-gray-100 p-1.5 z-50 animate-in fade-in zoom-in-95">
                    <div className="px-3 py-2 sm:hidden border-b border-gray-100 mb-1.5">
                      <p className="text-sm font-bold text-[#1A1F3D]">{user?.email}</p>
                      <p className="text-xs text-gray-500">Administrateur</p>
                    </div>
                    <button
                      onClick={() => setIsProfileMenuOpen(false)}
                      className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors font-medium text-left"
                    >
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-20 lg:p-8 lg:pb-8 relative">
          <Outlet />
        </main>
      </div>
        ...
        <div className="flex items-stretch">
          {navItems.map(({ name, path, Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex-1 flex flex-col items-center justify-center pt-2 pb-3 gap-1 text-[10px] font-semibold transition-colors ${
                  isActive ? "text-[#FF6600]" : "text-gray-400 active:text-gray-600"
                }`
              }
            >
              {({ isActive }) => (
